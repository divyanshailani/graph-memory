...
## [v3.7.0] - 2026-08-16
- **Batch Ingestion Engine (`engine.bulk_upsert_nodes`)**: One connection and one write transaction per file instead of per node, with full `get_or_create_node` semantics (history cap, trust MAX, observation merge) minus ledger noise. Full test-suite ingestion of this repo dropped from ~76s to ~5s (~17×). `ingest-code` now emits per-file batched upserts.
- **Cross-File Call Graph Resolution (`ingest._resolve_cross_file_calls`)**: Post-ingestion pass rewires same-file CALLS stubs to the uniquely-named definition in the same project namespace (identified structurally via DEFINED_IN edges). Ambiguous names keep their stubs; edge-less stubs are hard-deleted. Call graphs now cross file boundaries.
- **Identifier Substring Search**: `search_nodes` falls back to substring matching over node IDs when full-text tokenization can't help — fragments like `effective_trus` now find `calculate_effective_trust`. (An external-content trigram FTS5 index was tried and removed: its sync triggers intermittently corrupted under WAL on SQLite 3.50.4 with "database disk image is malformed"; the substring fallback has identical behavior with zero corruption surface.)
- **Contradiction Detection**: Agent-driven upserts that overwrite a scalar assertion (`description`/`summary`/`design_intent`) with a different value record a capped `conflicts` array on the node; mechanical AST re-verification never does. `graph-memory contradictions` lists them; snapshots surface the top 3 as `⚠ Conflicting Assertions`.
- **Stale-Node GC (`graph-memory prune`)**: Soft-decays decayed, unreferenced nodes (`--days`, `--min-trust`); Project roots are never pruned.
- **Real Reflection Engine**: `memory.reflect_session_memory` now digests the last 30 days of actual Decision_Ledger entries (filtered per category) plus live graph statistics into the 5 memory categories — replacing the v3.3 static boilerplate.
- **Repo Overhaul**: tests consolidated into `tests/` with a root `conftest.py`; `LICENSE` (MIT) added; `debug.py` and stale tracked artifacts (`.DS_Store`, egg-info) removed; `ISSUES.md` refreshed (OpenCode limitation resolved in v3.6.0); README overhauled for v3.7.0.
- **Regression Tests**: Added `tests/test_v3_7_0_scale_teams.py` (bulk semantics, cross-file calls, identifier search, contradiction detection incl. mechanical-upsert exclusion, prune, data-driven reflection, corruption-free repeated trigger updates). 57 tests passing.

## [v3.6.0] - 2026-08-16
- **Streamable HTTP MCP Transport (`graph-memory-mcp/http_server.py`)**: The full MCP server (all tools) now also runs over streamable HTTP via `graph-memory-mcp-http` (default `http://127.0.0.1:8765/mcp`, configurable via `GRAPH_MEMORY_HTTP_HOST/PORT`). Stateless session mode supports multiple concurrent agents on one endpoint; `/health` endpoint for liveness. Unlocks OpenCode (remote-MCP-only), Dockerized agents, and centrally hosted team memory. New `http` extra (`uvicorn`, `starlette`) included in `[all]`. Verified live: MCP initialize handshake + full tools/list over HTTP.
- **Memory Importers (`graph_memory/core/importers.py`)**: `graph-memory import-md` imports CLAUDE.md / AGENTS.md / .cursorrules / any markdown memory — each section becomes a Knowledge_Node with bullet observations, linked to a document node, with stable idempotent IDs. `graph-memory import-mem0` imports mem0 JSON exports (bare list, `{"results": [...]}`, or `{"memories": [...]}`; bare-string records supported) into Fact_Nodes. Migration cost from existing memory tooling: zero.
- **Obsidian Vault Export (`graph_memory/core/obsidian.py`)**: `graph-memory export-obsidian <dir>` writes curated knowledge (everything except mechanical AST nodes) as markdown notes with YAML frontmatter, observation bullets, and `[[Wikilinks]]` for graph edges (directional, relation-typed).
- **Monorepo-Safe `--root` Flag**: `ingest-code`, `ingest-file` (CLI and MCP `ingest_file` tool) accept an explicit namespace root. All relative paths are keyed to the namespace root, so both entrypoints derive identical node IDs even with nested project markers (pnpm workspaces, multi-package repos).
- **OpenCode Native MCP**: `hook install --framework opencode` now also registers a remote MCP entry (`graph-memory` → `http://127.0.0.1:8765/mcp`, overridable via `GRAPH_MEMORY_HTTP_URL`) in `opencode.json`; uninstall removes it.
- **Regression Tests**: Added `test_v3_6_0_open_borders.py` (markdown import idempotency, mem0 import, Obsidian wikilinks + AST exclusion, `--root` ID parity between ingest-code and ingest-file, HTTP app construction). 50 tests passing.
