...
@mcp.tool()
def add_node(workspace_dir: str, id: str, type: str, attributes: str = "{}") -> str:
    """
    ...
    return f"Successfully added node: {id} (Type: {type}) to workspace {workspace_dir}"

@mcp.tool()
def add_relation(workspace_dir: str, source: str, relation: str, target: str, attributes: str = "{}") -> str:
    """
    ...
    return f"Successfully added relation: {source} -[{relation}]-> {target} in workspace {workspace_dir}"
...
if __name__ == "__main__":
    # Removed global init_db() which crashed the server if running from a read-only root directory.
    mcp.run()
